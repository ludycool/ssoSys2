...
var s = ["province", "city", "locality"];//三个select的name
var opt0 = ["省份", "地级市", "市、县级市"];//初始值
function change(v) {


    var ss = document.getElementById(s[0]);
    var cc = document.getElementById(s[1]);
    var aa = document.getElementById(s[2]);

    switch (v) {

        case 0:
            ss.options.length = 0;//删除所有
            for (var i = 0; i < dsy.p.length; i++) {
                ss.options.add(new Option(dsy.p[i], dsy.p[i]));
            }


            break;
        case 1:
            cc.options.length = 0;//删除所有
            var index = ss.selectedIndex;
            var ssslect = ss.options[index].value;

            var ccata = dsy.c[ssslect]
            for (var i = 0; i < ccata.length; i++) {
                cc.options.add(new Option(ccata[i], ccata[i]));
            }


            break;
        case 2:
            aa.options.length = 0;//删除所有
            var index = cc.selectedIndex;
            var aaslect = cc.options[index].value;

            var cindex = ss.selectedIndex;
            var ssslect = ss.options[cindex].value;


            if (aaslect != undefined) {
                var aaata = dsy.a[ssslect + "-" + aaslect]
                for (var i = 0; i < aaata.length; i++) {
                    aa.options.add(new Option(aaata[i], aaata[i]));
                }
            }
            break;
    }
